...
(() => {
  ...
  // v12.2.5 — capture-phase SMART reward trigger.
  // This runs before any existing button handler, so the reward cannot be skipped
  // by renderer rebinding or legacy event listeners.
  if (!window.__KARANGAN_SMART_CAPTURE_V125__) {
    window.__KARANGAN_SMART_CAPTURE_V125__ = true;

    document.addEventListener("click", event => {
      const button = event.target.closest?.("[data-l2m-master]");
      if (!button) return;

      // Show visual reward immediately, before any other mastery logic runs.
      try {
        l2mShowMasterReward(true);
      } catch (error) {
        console.warn("SMART capture reward failed:", error);

        // Last-resort visible fallback: this must appear even if the image fails.
        document.getElementById("l2m-smart-overlay-v125")?.remove();

        const overlay = document.createElement("div");
        overlay.id = "l2m-smart-overlay-v125";
        overlay.setAttribute("style",
          "position:fixed;inset:0;z-index:2147483647;" +
          "display:flex;align-items:center;justify-content:center;" +
          "background:rgba(12,16,35,.42);pointer-events:none;"
        );
        ...
        overlay.innerHTML = `
          ...
          ">
            ...
            />
            <div style="font-size:24px;font-weight:950">Syabas! Sudah Kuasai!</div>
          </div>
        `;
  ...
  console.log("✅ MASTER CURRICULUM v12.2.5 + CAPTURE SMART REWARD loaded");
})();
