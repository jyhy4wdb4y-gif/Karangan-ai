...
(() => {
  ...
  function renderL2AllWords(){
    const e=l2Engine();
    const year=l2Year();
    const saved=[...getVocabularyWords()];
    const savedByWord=new Map(saved.map(w=>[l2NormalizePhrase(w.word),w]));

    // IMPORTANT: "Semua Kosa Kata" is the official Master Curriculum view.
    // LocalStorage/saved words remain progress records only and never become
    // the curriculum source for this screen.
    const master=(e?.getMasterWordBank?.()||[])
      .filter(item=>Number(item?.year||item?.minYear||3)===Number(year))
      .sort((a,b)=>{
        const ai=String(a?.id||a?.bankId||"");
        const bi=String(b?.id||b?.bankId||"");
        return ai.localeCompare(bi,'en',{numeric:true}) ||
          String(a?.word||"").localeCompare(String(b?.word||""),'ms');
      });

    const cards=master.map((item,index)=>{
      const progress=savedByWord.get(l2NormalizePhrase(item.word));
      const mastered=Boolean(progress?.mastered);
      const curriculumId=item.id||item.bankId||`T${year}-${String(index+1).padStart(3,'0')}`;
      const translation=item.translation||[item.zh,item.en].filter(Boolean).join(' · ');
      return `<article style="padding:16px 17px;margin-bottom:11px;border:1px solid #ece8e1;border-radius:18px;background:#fff">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px">
          <div>
            <div style="font-size:12px;font-weight:950;letter-spacing:.06em;color:#d97824">${escapeHtml(curriculumId)}</div>
            <strong style="display:block;font-size:20px;line-height:1.35;margin-top:3px">${escapeHtml(item.word||"")}</strong>
          </div>
          <span style="font-size:11px;font-weight:900;padding:5px 8px;border-radius:999px;background:${mastered?'#eaf8ef':'#f7f5ff'};color:${mastered?'#2d8556':'#6b55d9'}">${mastered?'✓ DIKUASAI':'MASTER'}</span>
        </div>
        ${translation?`<div style="color:#6b55d9;font-weight:800;margin-top:7px">${escapeHtml(translation)}</div>`:''}
        ${item.meaning?`<p style="color:#65727a;line-height:1.55;margin:9px 0 0">🇲🇾 ${escapeHtml(item.meaning)}</p>`:''}
        ${item.example?`<p style="background:#faf8ff;padding:10px;border-radius:12px;margin:10px 0 0">📝 ${escapeHtml(item.example)}</p>`:''}
      </article>`;
    }).join('');

    openModuleScreen(`<span class="section-kicker">LANGKAH 2 · TAHUN ${year}</span>
      <h1>📚 Semua Kosa Kata</h1>
      ${l2YearSelector(year)}
      <div style="padding:13px 14px;border-radius:14px;background:#eef9f3;color:#35634c;margin:0 0 18px;font-weight:800">✅ Curriculum Master FINAL · Tahun ${year} · ${master.length} item</div>
      <p style="color:#65727a;line-height:1.65">Senarai ini datang terus daripada Master Curriculum. Rekod lama dalam peranti hanya digunakan untuk status pembelajaran dan tidak lagi menentukan kandungan senarai ini.</p>
      ${cards||'<div style="padding:18px;background:#fff7e7;border-radius:16px">Master Curriculum untuk Tahun ini belum tersedia dalam engine.</div>'}
      <button id="l2BackToday" class="primary-button" type="button" style="width:100%;margin-top:18px">← Kembali ke Hari Ini</button>`,28);
