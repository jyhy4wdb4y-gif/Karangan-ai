...
(function(){
  ...
  function l3Mission(word,level,scaffold){
    const x=l3StateFor(word);
    if(level==="ASAS") return (x.depth||0)<=1 ? "LENGKAP" : "SUSUN";
    if(level==="STANDARD") return "SUSUN";
    return x.state==="WRITING_MASTERED" ? "CANTIKKAN" : "BINA";
  }
    ...
    openModuleScreen(`
      <div style="max-width:760px;margin:0 auto">
        <div style="text-align:center;padding:8px 0 18px">
          ...
          <div style="color:#666">Pilih tahun dan tema untuk misi menulis hari ini.</div>
        </div>
  ...
  function l3RenderScreen(){
    ...
    if(missionType==="PILIH"){
      ...
      mission=`<div style="padding:16px;border-radius:18px;background:#f7f5ff;margin:14px 0">
        <strong>🔀 Susun Ayat <span style="font-size:12px;font-weight:600;color:#777">· Tahap 1</span></strong>
  ...
  function l3BindStageActions(){
    ...
    document.addEventListener("pointerup",function(e){
      ...
      const next=e.target.closest?.("#l3Next");
      ...
      if(retry){
        try{
          ...
          if(l3Level()==="ASAS"){
            try{ completeMission("sentence-builder"); }catch(err){console.warn("[L3 Asas complete]",err);}
            ...
            const t=l3Theme(),done=l3ThemeCount>=L3_THEME_MAX;
            openModuleScreen(`<div style="text-align:center;padding:18px 0"><div style="font-size:64px">🌟</div>
              <h1>Bagus! Kamu berjaya.</h1>
              <p>Kamu sudah memahami dan menyusun ayat dengan betul.</p>
              <p style="color:#666">🌱 Asas: bina ayat sendiri belum diwajibkan.</p></div>
              <div style="display:grid;gap:10px">${done?`<button id="l3ThemeDone" class="primary-button">🏆 Selesai Hari Ini</button>`:
              `<button id="l3ThemeContinue" class="primary-button">${t.icon} Perkataan Seterusnya</button>
               <button id="l3ThemeDone" class="secondary-button">✓ Selesai</button>`}</div>`,100);
            return;
          }
      ...
      if(next){
        try{ completeMission("sentence-builder"); }catch(err){console.warn("[L3 completeMission]",err);}
        l3ThemeCount=Math.min(L3_THEME_MAX,l3ThemeCount+1);
        l3LastWordId=sentenceBuilderState?.l3Word?.id||"";
        sessionStorage.setItem("karangan_ai_l3_theme_count",String(l3ThemeCount));
        sessionStorage.setItem("karangan_ai_l3_last_word",l3LastWordId);
        const t=l3Theme(),done=l3ThemeCount>=L3_THEME_MAX;
        openModuleScreen(`<div style="text-align:center;padding:18px 0"><div style="font-size:64px">🌟</div>
          <h1>${l3ThemeCount===1?"Misi Hari Ini Selesai!":"Bagus! Satu Lagi Berjaya!"}</h1>
          <p>Kamu sudah berjaya menggunakan perkataan dalam tema <strong>${escapeHtml(t.name)}</strong>.</p></div>
          <div style="display:grid;gap:10px">
          ${done?`<button id="l3ThemeDone" class="primary-button">🏆 Selesai Hari Ini</button>`:
          `<button id="l3ThemeContinue" class="primary-button">${t.icon} Perkataan Seterusnya</button>
           <button id="l3ThemeDone" class="secondary-button">✓ Selesai</button>`}</div>`,100);
      }
    },true);
  ...
  function l3CheckDropMission(){
    ...
    const sample=String(l3Example(w,Math.max(1,Math.min(3,st.l3Scaffold)))||"")
      .replace(/[.!?]/g,"").trim();
  ...
  function l3Feedback(result,praise,feedback,success,guided){
    ...
    openModuleScreen(`<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:8px 10px;border-radius:14px;background:#f7f7f7;margin-bottom:10px">
      ...
      ${success&&!guided
        ?`<button id="l3Next" class="primary-button" style="width:100%;margin-top:14px">Teruskan →</button>`
        :(l3Level()==="ASAS"
          ?`<div style="display:grid;gap:9px;margin-top:14px">
              <button id="l3AsasContinue" class="primary-button" style="width:100%">${l3Theme().icon} Cabar Perkataan Seterusnya</button>
              <button id="l3AsasDone" class="secondary-button" style="width:100%">✓ Selesai</button>
            </div>`
          :`<button id="l3Retry" class="primary-button" style="width:100%;margin-top:14px">✍️ Cuba Bina Ayat Sendiri</button>`)}`,42);
  }
  ...
  function l3CompleteAsasWord(goNext){
    try{
      if(!sentenceBuilderState?.l3Word){showToast("⚠️ Misi belum sedia.");return;}
      try{completeMission("sentence-builder");}catch(err){console.warn("[L3 Asas complete]",err);}
      l3ThemeCount=Math.min(L3_THEME_MAX,l3ThemeCount+1);
      l3LastWordId=sentenceBuilderState.l3Word.id;
      sessionStorage.setItem("karangan_ai_l3_theme_count",String(l3ThemeCount));
      sessionStorage.setItem("karangan_ai_l3_last_word",l3LastWordId);
      if(goNext && l3ThemeCount<L3_THEME_MAX){
        sentenceBuilderState.l3Word=null;
        sentenceBuilderState.l3Mission=null;
        sentenceBuilderState.attempts=0;
        sentenceBuilderState.hintLevel=0;
        l3Render();
        return;
      ...
      openModuleScreen(`<div style="text-align:center;padding:22px 0">
        ...
        <h1>${l3ThemeCount>=L3_THEME_MAX?"Hebat! Latihan Hari Ini Lengkap.":"Misi Hari Ini Selesai!"}</h1>
        <p>Kamu sudah berjaya dalam tema <strong>${escapeHtml(t.name)}</strong>.</p>
      </div>
      ${l3ThemeCount<L3_THEME_MAX?`<button id="l3ThemeContinue" class="primary-button" style="width:100%">${t.icon} Perkataan Seterusnya</button>`:""}
      <button id="l3ThemeDone" class="secondary-button" style="width:100%;margin-top:9px">✓ Selesai</button>`,100);
    }catch(err){
      console.error("[L3 Asas challenge]",err);
      showToast("⚠️ Cuba sekali lagi.");
    }
  }

  let l3AsasActionAt=0;
  function l3AsasAction(e){
    const el=e.target.closest?.("#l3AsasContinue,#l3AsasDone");
    if(!el)return;
    const now=Date.now(); if(now-l3AsasActionAt<350)return; l3AsasActionAt=now;
    e.preventDefault();e.stopPropagation();
    l3CompleteAsasWord(el.id==="l3AsasContinue");
  }
  document.addEventListener("pointerup",l3AsasAction,true);
  document.addEventListener("click",l3AsasAction,true);

  let l3ThemeActionAt=0;
  function l3ThemeAction(e){
    const el=e.target.closest?.("[data-l3-theme],[data-l3-theme-start],[data-l3-year],#l3StartJourney,#l3ChangeTheme,#l3ThemeContinue,#l3ThemeDone"); if(!el)return;
    ...
    if(el.id==="l3StartJourney"){l3JourneyStarted=true;l3SessionLevel=null;l3Render();return;}
    if(el.id==="l3ChangeTheme"){l3JourneyStarted=false;l3RenderStartPage();return;}
    if(el.id==="l3ThemeContinue"){l3SessionLevel=null;l3Render();return;}
  ...
  window.KaranganLangkah3={
    version:L3_VERSION,render:l3EntryRender,getState:l3Load,
