...
(function(){
  ...
  function l3Level(word){
    // Automatic adaptive route. Manual value is only a temporary developer override.
    try {
      const override=localStorage.getItem(L3_LEVEL_KEY);
      if(override==="ASAS"||override==="STANDARD"||override==="LANJUTAN") return override;
    } catch(_) {}
  ...
  function l3Header(level,word){
    ...
    return `<span class="section-kicker">LANGKAH 3 · TAHUN 1</span>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border-radius:14px;background:#f7f5ff;margin:14px 0">
        <span>🤖 Cikgu Aira pilih cabaran yang sesuai</span>
        <strong>${label}</strong>
      </div>
  ...
  function l3Render(){
    ...
    const word=l3Pick(), level=l3Level(word), scaffold=l3Scaffold(word,level);
  ...
  window.KaranganLangkah3={
    ...
    clearLevelOverride:function(){try{localStorage.removeItem(L3_LEVEL_KEY);}catch(_){}}
  };
})();
