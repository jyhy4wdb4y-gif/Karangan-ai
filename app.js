...
(function(){
  ...
  function l3SafeIndependentSuccess(answer,w){
    ...
    try{
      const state=l3Load(), x=state[w.id]||l3StateFor(w);
      x.seen=(x.seen||0)+1;x.postponed=false;x.independent=(x.independent||0)+1;
      x.depth=Math.max(x.depth||0,x.independent>=2?3:2);
      x.state=x.depth>=3?"WRITING_LEARNING":"LEARNING";
      x.nextReview=l3NextReview(x.depth||1); state[w.id]=x;l3Save(state);
  ...
  function l3BindStageActions(){
    ...
    document.addEventListener("pointerup",function(e){
      ...
      if(next){
        try{
          completeMission("sentence-builder");
        }catch(err){ console.warn("[L3 completeMission]",err); }
        l3SessionLevel=null;
        l3Render();
      }
    },true);
  ...
  function l3CheckDropMission(){
    ...
    try{
      const state=l3Load(), x=state[w.id]||l3StateFor(w);
      x.seen=(x.seen||0)+1;x.postponed=false;x.state="WRITING_LEARNING";
      x.depth=Math.max(x.depth||0,2);x.nextReview=l3NextReview(x.depth);
      state[w.id]=x;l3Save(state);
