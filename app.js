...
(function(){
  ...
  function l3Mission(word,level,scaffold){
    const x=l3StateFor(word);
    // v12.9.3: preserve the child-facing two-stage flow.
    // Stage 1 is scaffolded (PILIH/LENGKAP/SUSUN); Stage 2 becomes independent BINA.
    // Mastery depth must not silently remove Drag & Drop from the normal experience.
    if(level==="ASAS"){
      return (x.depth||0)<=1 ? "LENGKAP" : "SUSUN";
    }
    if(level==="STANDARD"){
      return "SUSUN";
    }
    // Lanjutan can still start with SUSUN unless already Writing Mastered.
    return x.state==="WRITING_MASTERED" ? "CANTIKKAN" : "SUSUN";
  }
  ...
  function l3Feedback(result,praise,feedback,success,guided){
    ...
    openModuleScreen(`<div style="padding:12px;border-radius:16px;background:#f4f9ff;margin-bottom:10px">
      <div style="font-weight:800">📘 Tahun Pembelajaran</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">${l3YearButtons()}</div>
      <div style="font-size:12px;color:#666;margin-top:6px">Tahun 2–6 akan dibuka selepas kandungan kurikulum disediakan.</div>
    </div>
    <div style="padding:12px;border-radius:16px;background:#f7f7f7;margin-bottom:12px">
      <div style="font-weight:800">${l3Theme().icon} Tema Hari Ini: ${escapeHtml(l3Theme().name)}</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">${Object.entries(L3_THEMES).map(([k,t])=>`<button type="button" data-l3-theme="${k}" style="padding:6px 9px;border-radius:14px;border:1px solid #ddd;background:${k===l3ThemeKey?"#fff3df":"#fff"}">${t.icon} ${escapeHtml(t.name)}</button>`).join("")}</div>
      ${l3ThemeCount?`<div style="font-size:12px;color:#666;margin-top:6px">Latihan tema: ${l3ThemeCount}/${L3_THEME_MAX}</div>`:""}
      ...
      ${success&&!guided?`<button id="l3Next" class="primary-button" style="width:100%;margin-top:14px">Teruskan →</button>`:`<button id="l3Retry" class="primary-button" style="width:100%;margin-top:14px">✍️ Cuba Bina Ayat Sendiri</button>`}`,42);
  }

  renderSentenceBuilder=l3Render;
  let l3ThemeActionAt=0;
  function l3ThemeAction(e){
    const el=e.target.closest?.("[data-l3-theme],[data-l3-year],#l3ThemeContinue,#l3ThemeDone"); if(!el)return;
