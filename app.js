...
(function(){
  ...
  function l3Mission(word,level,scaffold){
    const x=l3StateFor(word);
    // Hidden progression: PILIH → LENGKAP → SUSUN → BINA → CANTIKKAN.
    // Difficulty level changes support, not curriculum scope.
    if(scaffold<=1 && (x.depth||0)<=1) return "PILIH";
    if((x.depth||0)<=1) return "LENGKAP";
    if((x.depth||0)===2 && (x.independent||0)<1) return "SUSUN";
    if((x.depth||0)<=3 && level!=="LANJUTAN") return "BINA";
    return "CANTIKKAN";
  }
  ...
  function l3Render(){
    if(l3Year()!==1) return L3_LEGACY_RENDER();
  ...
  function l3RenderScreen(){
    ...
    const ex=l3Example(word,Math.max(1,Math.min(3,scaffold)));
    if(missionType==="PILIH"){
      ...
      mission=`<div style="padding:16px;border-radius:18px;background:#f7f5ff;margin:14px 0">
        ...
        <textarea id="l3Answer" rows="4" placeholder="Tulis ayat yang lebih lengkap..." style="width:100%;box-sizing:border-box;border:2px solid #e4dfda;border-radius:16px;padding:14px;font:inherit"></textarea>
        <button id="l3Check" class="primary-button" type="button" style="width:100%;margin-top:12px">✓ Semak Ayat</button>`;
    } else {
      mission=`<div style="padding:16px;border-radius:18px;background:#f7f5ff;margin:14px 0">
        <strong>✍️ Bina Ayat</strong><div style="margin-top:8px">Bina satu ayat menggunakan <strong>${escapeHtml(word.word)}</strong>.</div></div>
        <textarea id="l3Answer" rows="4" placeholder="Tulis ayat kamu di sini..." style="width:100%;box-sizing:border-box;border:2px solid #e4dfda;border-radius:16px;padding:14px;font:inherit"></textarea>
        <button id="l3Check" class="primary-button" type="button" style="width:100%;margin-top:12px">✓ Semak Ayat</button>`;
    }
    let hint="";
    if(st.hintLevel){
    ...
    byId("l3Skip")?.addEventListener("click",()=>{
      const s=l3Load(), x=s[word.id]||l3StateFor(word); x.postponed=true;x.state=x.state==="NEW"?"LEARNING":x.state;s[word.id]=x;l3Save(s);
  ...
  function l3BindSemakDelegation(){
    const root=byId("moduleContent");
    if(!root || root.dataset.l3SemakBound==="1") return;
    root.dataset.l3SemakBound="1";
  ...
  function l3CheckDropMission(){
    const st=sentenceBuilderState, w=st?.l3Word;
    if(!st || !w){showToast("⚠️ Misi belum sedia. Cuba buka semula Bina Ayat.");return;}
    const zone=byId("moduleContent")?.querySelector(".l3-dropzone");
    const dropped=zone ? [...zone.querySelectorAll(".l3-chip")].map(x=>x.dataset.word||x.textContent.trim()) : [];
    if(!dropped.length){showToast("👆 Seret perkataan ke kotak dahulu.");return;}
    if(st.l3Mission==="LENGKAP"){
      l3Evaluate(dropped[0],true);
      return;
    }
    if(st.l3Mission==="SUSUN"){
      const expected=l3ShuffleTokens(l3Example(w,Math.max(1,Math.min(3,st.l3Scaffold))));
      // Compare normalized token sequence with the original example sentence, not shuffled display.
      const original=String(l3Example(w,Math.max(1,Math.min(3,st.l3Scaffold)))||"")
        .replace(/[.!?]/g,"").split(/\s+/).filter(Boolean).map(l3Norm);
      const actual=dropped.map(l3Norm);
      const correct=actual.length===original.length && actual.every((x,i)=>x===original[i]);
      const s=l3Load(), x=s[w.id]||l3StateFor(w); x.seen=(x.seen||0)+1; x.postponed=false;
      if(!correct){
        x.retries=(x.retries||0)+1; x.state="LEARNING"; x.depth=Math.max(x.depth||0,1);
        x.nextReview=l3NextReview(x.depth||1); s[w.id]=x; l3Save(s);
        l3Feedback("RETRY","👍 Hampir betul!","Cuba susun semula perkataan mengikut urutan ayat yang betul.",false);
        return;
      }
      // SUSUN is always guided: success advances learning but never independent mastery.
      x.state="WRITING_LEARNING"; x.depth=Math.max(x.depth||0,2); x.nextReview=l3NextReview(x.depth);
      s[w.id]=x; l3Save(s);
      l3Feedback("SUCCESS","🌟 Susunan betul!","Bagus! Sekarang cuba bina ayat sendiri tanpa susunan perkataan.",true,true);
    }
  }
  function l3Evaluate(answer,isChoice){
    const st=sentenceBuilderState, w=st.l3Word, scaffold=st.l3Scaffold;
    const raw=String(answer||"").trim(), norm=l3Norm(raw), target=l3Norm(w.word);
    if(!raw){showToast("✍️ Tulis jawapan dahulu.");return;}
    const contains=isChoice ? norm===target : norm.split(" ").includes(target);
    const words=norm.split(/\s+/).filter(Boolean);
    let issue="";
    if(!contains) issue="TARGET_WORD";
    else if(!isChoice && words.length<2) issue="TOO_SHORT";
    else if(!isChoice && raw[0] && raw[0]!==raw[0].toUpperCase()) issue="CAPITAL";
    else if(!isChoice && !/[.!?]$/.test(raw)) issue="PUNCTUATION";
    else if(st.l3Mission==="CANTIKKAN" && words.length<=l3Norm(w.examples[0]).split(/\s+/).length) issue="EXPAND";
    ...
    if(issue){
      ...
      s[w.id]=x;l3Save(s);
      const msg=issue==="TARGET_WORD"?"Cuba gunakan perkataan sasaran dalam ayat kamu."
        :issue==="TOO_SHORT"?"Cuba bina ayat yang lebih lengkap."
        :issue==="CAPITAL"?"Cuba mula ayat dengan huruf besar."
        :issue==="EXPAND"?"Tambah satu maklumat lagi supaya ayat lebih jelas."
        :"Cuba letakkan tanda noktah di hujung ayat.";
    ...
    x.nextReview=l3NextReview(x.depth||1);
    s[w.id]=x;l3Save(s);
    l3Feedback("SUCCESS",guided?"🌟 Bagus!":"🌟 Hebat!",guided?"Sekarang cuba lagi tanpa melihat contoh.":"Ayat kamu jelas dan menggunakan perkataan sasaran.",true,guided);
  }
  function l3Feedback(result,praise,feedback,success,guided){
    const w=sentenceBuilderState.l3Word;
    openModuleScreen(`<span class="section-kicker">CIKGU AIRA</span>
      ...
      ${success&&!guided?`<button id="l3Next" class="primary-button" style="width:100%;margin-top:14px">Teruskan →</button>`:`<button id="l3Retry" class="primary-button" style="width:100%;margin-top:14px">✍️ Cuba Sekali Lagi</button>`}`,42);
    byId("l3Next")?.addEventListener("click",()=>{completeMission("sentence-builder");l3SessionLevel=null;l3Render();});
    byId("l3Retry")?.addEventListener("click",()=>{
      sentenceBuilderState.hintLevel=0;
      sentenceBuilderState.exampleRevealed=false;
      sentenceBuilderState.l3Scaffold=Math.max(2,sentenceBuilderState.l3Scaffold);
      sentenceBuilderState.l3Mission="BINA";
      l3RenderScreen();
    });
  }

  renderSentenceBuilder=l3Render;
  window.KaranganLangkah3={
    version:L3_VERSION,render:l3Render,getState:l3Load,
