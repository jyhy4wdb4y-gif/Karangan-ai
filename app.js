...
(function(){
  ...
  function l3ShortExample(word,level){
    ...
    // Tahun 1 difficulty is controlled by the selected level, not by prior mastery.
    // ASAS deliberately uses a tiny 3-token puzzle wherever possible.
    const asas={
    ...
    const standard={
      membantu:"Saya membantu ibu di rumah.", membaca:"Saya membaca buku di rumah.", menulis:"Saya menulis di dalam kelas.",
      bermain:"Saya bermain bersama kawan saya.", belajar:"Saya belajar di dalam kelas.", menjaga:"Saya menjaga adik di rumah.",
      membersihkan:"Saya membersihkan kelas bersama kawan.", mengemas:"Saya mengemas bilik saya sendiri.",
      melihat:"Saya melihat bunga yang cantik.", pergi:"Saya pergi ke sekolah pagi.",
      pulang:"Saya pulang ke rumah petang.", gembira:"Saya gembira bersama keluarga saya."
    };
    const lanjutan={
      membantu:"Saya membantu ibu mengemas rumah.", membaca:"Saya membaca buku bersama kawan saya.",
      menulis:"Saya menulis ayat di dalam kelas.", bermain:"Saya bermain bersama kawan di taman.",
      belajar:"Saya belajar bersama kawan di sekolah.", menjaga:"Saya menjaga adik di rumah dengan baik.",
      melihat:"Saya melihat bunga cantik di taman.", gembira:"Saya gembira bermain bersama kawan saya."
    };

    let text=(level==="ASAS"?asas[w]:(level==="STANDARD"?(standard[w]||asas[w]):(lanjutan[w]||standard[w]||asas[w])));
    if(!text) text=l3Example(word,1);

    const cap=level==="ASAS"?4:(level==="STANDARD"?5:6);
    let words=String(text).replace(/[.!?]/g,"").trim().split(/\s+/).filter(Boolean);

    // Absolute safety cap: never allow a selected level to inherit a longer legacy example.
    if(words.length>cap){
      const candidates=(word?.examples||[]).filter(Boolean).filter(x=>String(x).toLowerCase().includes(w.toLowerCase()));
      const fit=candidates.find(x=>String(x).replace(/[.!?]/g,"").trim().split(/\s+/).filter(Boolean).length<=cap);
      if(fit) text=fit;
      else if(level==="ASAS") text=asas[w]||text;
    }
    return text;
  }
  ...
  function l3RenderScreen(){
    ...
    if(missionType==="PILIH"){
      ...
      mission=`<div style="padding:16px;border-radius:18px;background:#f7f5ff;margin:14px 0">
        <strong>🔀 Susun Ayat <span style="font-size:12px;font-weight:600;color:#777">· ${level==="ASAS"?"🌱 Asas":level==="STANDARD"?"🌿 Standard":"⭐ Lanjutan"}</span></strong>
