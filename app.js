...
(function(){
  ...
  function l3ShortExample(word,level){
    ...
    const shortBank={
      keluarga:"Ini keluarga saya.", ibu:"Saya sayang ibu.", ayah:"Saya sayang ayah.", adik:"Saya menjaga adik.",
      bersama:"Kami bermain bersama.", sayang:"Saya sayang ibu.", membantu:"Saya membantu ibu.", berkongsi:"Kami berkongsi buku.",
      menjaga:"Saya menjaga adik.", gembira:"Saya berasa gembira.", sekolah:"Saya pergi sekolah.", guru:"Guru mengajar saya.",
      kawan:"Dia kawan saya.", kelas:"Kelas saya bersih.", belajar:"Saya suka belajar.", membaca:"Saya suka membaca.",
      menulis:"Saya suka menulis.", bermain:"Saya suka bermain.", rajin:"Ali murid rajin.", baik:"Ali sangat baik.",
      rumah:"Ini rumah saya.", bilik:"Bilik saya bersih.", dapur:"Ibu di dapur.", makan:"Saya makan nasi.",
      minum:"Saya minum air.", mengemas:"Saya mengemas bilik.", membersihkan:"Saya membersihkan kelas.", tidur:"Saya mahu tidur.",
      bangun:"Saya bangun pagi.", mandi:"Saya sudah mandi.", sedih:"Saya berasa sedih.", takut:"Saya berasa takut.",
      seronok:"Saya berasa seronok.", penat:"Saya berasa penat.", berani:"Ali sangat berani.", bersih:"Kelas saya bersih.",
      cantik:"Bunga itu cantik.", besar:"Rumah itu besar.", kecil:"Bola itu kecil.", suka:"Saya suka membaca.",
      dan:"Saya membaca dan menulis.", kerana:"Saya gembira kerana berjaya.", pagi:"Saya bangun pagi.", petang:"Saya bermain petang.",
      pergi:"Saya pergi sekolah.", pulang:"Saya pulang rumah.", membuat:"Saya membuat kerja.", melihat:"Saya melihat bunga.",
      mempunyai:"Saya mempunyai buku.", kemudian:"Saya makan kemudian."
    };
    let text=shortBank[w]||l3Example(word,1);
    const cap=level==="ASAS"?4:(level==="STANDARD"?5:6);
    const count=String(text).replace(/[.!?]/g,"").trim().split(/\s+/).filter(Boolean).length;
    if(count>cap){
      const candidates=(word?.examples||[]).filter(Boolean).sort((a,b)=>
        String(a).replace(/[.!?]/g,"").trim().split(/\s+/).length-
        String(b).replace(/[.!?]/g,"").trim().split(/\s+/).length);
      const fit=candidates.find(x=>{
        const n=String(x).replace(/[.!?]/g,"").trim().split(/\s+/).filter(Boolean).length;
        return n<=cap && String(x).toLowerCase().includes(w.toLowerCase());
      });
      if(fit) text=fit;
    }
    return text;
